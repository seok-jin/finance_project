...
class Get_stock_ticker:
    ...
    def get_stock_data(self):
        # 코스피
        df_KOSPI = stock.get_market_ohlcv_by_ticker(self.to_day, market="KOSPI")
        df_KOSDAQ = stock.get_market_ohlcv_by_ticker(self.to_day, market="KOSDAQ")
            ...
            if i is not None:
                ...
                if os.path.isfile('./stock_data_ticker/KOSPI/KOSPI_ticker_list_' + i + '.scv'):
                    ...
                    if csv_data.iloc[-1]['날짜'].replace('-', '') < str(self.to_day):
                        ...
                        a.insert(0, date.today().isoformat())
                        if a[-2] != 0:
                            csv_data.loc[len(csv_data)] = a
                            csv_data.to_csv('./stock_data_ticker/KOSPI/KOSPI_ticker_list_' + i + '.scv')
                            update_count += 1
                        else:
                            no_count += 1
                    else:
        ...
        self.logging.logger.debug(
            "KOSPI 종목별 히스토리 수집 종료 신규 : " + str(insert_count) + ", 업데이트 : " + str(update_count) + ", 변경없음 : " + str(no_count))
        ...
        for i in np.array(stock_list_data['0'].tolist()):
            if i is not None:
                ...
                if os.path.isfile('./stock_data_ticker/KOSDAQ/KOSDAQ_ticker_list_' + i + '.scv'):
                    ...
                    insert_count += 1
        self.logging.logger.debug(
            "KOSDAQ 종목별 히스토리 수집 종료 신규 : " + str(insert_count) + ", 업데이트 : " + str(update_count) + ", 변경없음 : " + str(no_count))
